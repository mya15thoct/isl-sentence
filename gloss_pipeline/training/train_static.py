"""
Train MLP static classifier on ISL-Frames-Data keypoints.

Usage:
  python gloss_pipeline/training/train_static.py \
      --kp_dir /mnt/ngan/ISL-Sequences/frame_keypoints \
      --ckpt_dir /mnt/ngan/ISL-Sequences/checkpoints/static_classifier
"""
...
from models.static_classifier import build_static_classifier
...
def train(args):
    ...
    model = build_static_classifier(num_classes=len(class2id))
    model.compile(
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            filepath=str(ckpt_dir / 'best_static_classifier'),
    ...
    with open(ckpt_dir / 'class_mapping.json', 'w') as f:
        json.dump({'class2id': class2id, 'id2class': {str(k): v for k, v in id2class.items()}}, f, indent=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir',  default=str(ISL_SEQ_DIR / 'checkpoints' / 'static_classifier'))
